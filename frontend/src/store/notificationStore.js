			...
			fetchBudgetAlerts: async () => {
				...
				try {
					...
					const notifications = criticalAlerts.map((alert) => ({
						...
						type: 'budget_alert',
						title: `⚠️ Budget Exceeded: ${alert.category}`,
						message: `You've spent $${alert.spent.toFixed(2)} of your $${alert.budget_amount.toFixed(2)} ${alert.period} budget (${alert.percentage.toFixed(1)}%)`,
						severity: 'critical',
